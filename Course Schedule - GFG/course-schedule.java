...
class Solution
{
    static HashMap<Integer,Integer> fillmap (ArrayList<ArrayList<Integer>> prerequisites){
        //for each prerequisites pair create a weighted value for each time a course needs to be completed
    ...
    static HashMap<Integer,ArrayList<Integer>> fillmap2 (ArrayList<ArrayList<Integer>> prerequisites){
        //for each prerequisite create a graph o who should access the prerequisite ie: the reverse arcs of the prerequisite
    ...
    static void bfs(HashMap<Integer, Integer> completed, HashMap<Integer,ArrayList<Integer>> graph, ArrayList<Integer> sol, int init){
        //Execute the bfs adding to sol each visited node
        ...
        while(!q.isEmpty()){
            ...
            if(children!=null){//if it has any children
                for(int child: children){//for each child
                    if(completed.get(child)!=null){//if the child has prerequisites
                        completed.put(child, completed.get(child)-1);//one prerequisite down
                        if(completed.get(child)==0&&!sol.contains(child)){//if all prerequisites are done add it to the queue
                            q.add(child);
                        }
                    }
                }
    ...
    static int[] findOrder(int n, int m, ArrayList<ArrayList<Integer>> prerequisites) 
    {
        ...
        for(int i = 0; i<n; i++){
            if(completed.get(i)==null){//for each course that doesnt have prerequisites do bfs
